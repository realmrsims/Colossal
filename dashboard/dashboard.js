...
var url = req.protocol + '://' + req.get('host') + req.originalUrl;
    ...
    app.get("/support", (req, res) => {
      res.redirect(`https://discord.gg/hjKAnBH5Qk`)
    });
...
member.send(new MessageEmbed().setColor('GREEN').setFooter(`Powered by https://colossal.cf`).setTitle(`Application #${ticketID}`).setDescription(`Hey ${member.user.username}! Your form was Submitted and ready to be judged.\n\n**Form ID**: \`${ticketID}\`\n**Time:** ${moment(new Date()).format("dddd, MMMM Do YYYY HH:mm:ss")}`)).catch(()=>{});
...
channel.send(embed)
 ...
 app.get("/redeem", checkAuth, (req, res) => {
    ...
    app.get("/dashboard", checkAuth, (req, res) => {
      ...
      const server = client.guilds.cache.get('873534468693061652');
      ...
      if (Object.prototype.hasOwnProperty.call(data, "sendEmbed")) {
        ...
        if (ticketSettings.ticketToggle == "true") {
          ...
          if (ticketSettings.ticketType == "reaction" && ticketSettings.ticketCustom == "false") {
            ...
            if (storedSettings.isPremium == "false") {
              ticketSettings.ticketFooter = "Powered by colossal.cf";
            } else {
            ...
            let reactionFooter = "Powered by colossal.cf";



            let footer = "Powered by colossal.cf";
            ...
            if (storedSettings.isPremium == "false") {
              ticketEmbed.setFooter(`Powered by colossal.cf`)
            } else {
...
if(maintenance && maintenance.toggle == "true") {
    ...
    app.post('/dblwebhook', webhook.middleware(), async (req) => {
      ...
      const msg = new Discord.MessageEmbed()
        ...
        .setDescription(`Thank you **${apiUser.username}#${apiUser.discriminator}** (${apiUser.id}) for voting **Colossal**!`)
      Hook.send(msg);
...
if(config.datadogApiKey){
  metrics.init({ apiKey: jsonconfig.datadogApiKey, host: 'Colossal', prefix: 'p!' });
