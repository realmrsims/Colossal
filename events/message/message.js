...
module.exports = class extends Event {
  ...
  async run(message) {
    ...
    metrics.init({ apiKey: this.client.config.datadogApiKey, host: 'Colossal', prefix: 'Colossal.' });
      ...
      if (message.content.match(mentionRegex)) {
        ...
        const embed = new MessageEmbed()
          .setTitle('Hello, I\'m Colossal. What\'s Up?')
          .addField(`Prefix`,proofita, true)
          .addField(`Usage`,proofitaa, true)
          .setDescription(`\nIf you like Colossal, Consider [voting](https://top.gg/bot/767705905235099658), or [inviting](https://discord.com/oauth2/authorize?client_id=767705905235099658&scope=bot&permissions=470150262) it to your server! Thank you for using Colossal, we hope you enjoy it, as we always look forward to improve the bot`)
          .setFooter('Thank you for using Colossal!!')
          .setColor('#FF2C98')
...
message.channel.send(`Colossal is currently undergoing maintenance which won't allow anyone to access Colossal's Commands. Feel free to try again later. For updates: https://discord.gg/kTPGztA8Ka`)
...
return message.channel.send(embed)
  ...
  if (userBlacklistSettings && userBlacklistSettings.isBlacklisted)  return;
      ...
      if (command) {
        ...
        if (typeof rateLimit === "string") return message.channel.send(` ${message.client.emoji.fail} Please wait **${rateLimit}** before running the **${cmd}** command again - ${message.author}\n\n${number === 1 ? "*Did You know that Colossal has its own dashboard? `https://Colossal.cf/dashboard`*" : ""}${number === 2 ? "*You can check our top.gg page at `https://vote.Colossal.cf`*" : ""}`).then((s)=>{
