...
module.exports = class extends Command {
    ...
    async run(message, args) {
      ...
      const green = '<:modcheckgreen:951474548056948776>';
      const red = '<:modcheckred:951474548023386163>';
    ...
    const prefix = guildDB.prefix;
      ...
      if (!args || args.length < 1) {
       ...
       categories = this.client.utils.removeDuplicates(this.client.commands.filter(cmd => cmd.category !== 'Owner').map(cmd => cmd.category));
        ...
        embed.setTitle(`Colossal's Command List`)
        ...
        embed.setTimestamp()
        embed.setImage('https://cdn.discordapp.com/attachments/711487109294194769/951531172721225758/standard.gif')
      ...
      return message.channel.send(embed)
        ...
        embed.setDescription(this.client.commands.filter(cmd => 
            cmd.category.toLowerCase() === "owner").map(cmd => `${cmd.disabled || disabledCommands.includes(cmd.name || cmd) ? red : green} \`${cmd.name} ${" ".repeat(16 - Number(cmd.name.length))}:\` ${cmd.description}`).join("\n"));
      ...
      if(!message.channel.nsfw) embed.setDescription(`The following Menu only belongs to NSFW Channels 👀`)
        ...
        } else if(args && args.join(" ").toLowerCase() == "banners"  || args && args[0].toLowerCase() == "banners"){

          embed.setTitle(`Banners`)
          embed.setDescription(this.client.commands.filter(cmd => 
              cmd.category.toLowerCase() === "banners").map(cmd => `${cmd.disabled || disabledCommands.includes(cmd.name || cmd) ? red : green} \`${cmd.name} ${" ".repeat(9 - Number(cmd.name.length))}:\` ${cmd.description}`).join("\n"));
