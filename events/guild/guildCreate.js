...
logging2.moderation.mute_role = muteRole.id
    ...
    if(textChats){
      ...
      .setColor('PURPLE')
      .setDescription(`Hello! I'm **Colossal**.\n\nThank you for inviting me to your server as it means a lot to us! You can get started with [\`p!help\`](https://colossal.tk) & customise your server settings by accessing the Dashboard [\`here\`](https://colossal.cf/dashboard/${guild.id}).\n\n__**Current News**__\n\`\`\`\nWe are currently giving premium to all servers until 1000 guilds! If interested Please visit https://colossal.cf/redeem\`\`\`\n\nAgain, thank you for inviting me!\n**- Colossal**`)
      .addField(
        '\u200b', 
        '**[Invite](https://colossal.cf/invite) | ' +
        '[Support Server](https://colossal.cf/support) | ' +
        '[Dashboard](https://colossal.cf/dashboard)**'
      );
    ...
    .setColor(`PURPLE`)
    .setTitle('New Server')
    .setThumbnail(`https://Colossal.cf/logo`)
    .setDescription(`Colossal was added to a new Server!`)
    .addField(`Server Name`, `\`${guild.name}\``, true)
    .addField(`Server ID`, `\`${guild.id}\``, true)
    .setFooter(`${this.client.guilds.cache.size} guilds `,  'https://Colossal.cf/logo.png');

welcomeClient.send({
   username: 'Colossal',
        avatarURL: 'https://Colossal.cf/logo.png',
        embeds: [welcomeEmbed],
})
